...
void inicializar_iir_2_ord(float*num, float*den, float*w, float*k, coef_iir_2_ord* ir)
{
  for (int i = 0; i < 3; i++) {
    ir->num[i] = num[i];
    ir->den[i] = den[i];
...
float filtrarIIR(float in, coef_iir_2_ord* ir){
  ...
  float y = 0; //Variable auxiliar

  ir->w[0] = ( (1/(ir->den[0])) * ( (in*(ir->k[0])) - ((ir->den[1]) * (ir->w[1])) - ((ir->den[2]) * (ir->w[2])) ) );
  y = ((ir->num[0]) * (ir->w[0]))+((ir->num[1]) * (ir->w[1]))+((ir->num[2]) * (ir->w[2]));
  (ir->w[2]) = (ir->w[1]);//debe ser primero este
  (ir->w[1]) = (ir->w[0]);// y luego este para no sobreescribir
